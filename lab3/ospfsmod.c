...
allocate_block(void)
{
	...
	{
		if (bitvector_test(free_bitmap, i) == 1) 
		{
...
free_block(uint32_t blockno)
{
	...
	if (blockno < ospfs_super->os_firstinob) 
		return;
	void *free_bitmap = ospfs_block(OSPFS_FREEMAP_BLK);
	if (bitvector_test(free_bitmap, blockno) == 0)
...
add_block(ospfs_inode_t *oi)
{
	...
	else 
	{
		...
		uint32_t offset2 = indir_index(n);
		uint32_t indir = indir2[offset2];
		uint32_t *indir_ptr;
		if (indir == 0)
		{
			...
			memset(ospfs_block(allocated[0]), 0, OSPFS_BLKSIZE);
			indir = allocated[0];
			indir_ptr = ospfs_block(indir);
		}
		else
		{
			indir_ptr = ospfs_block(indir);
		}
		indir_ptr[direct_index(n)] = new_block;
	}
...
remove_block(ospfs_inode_t *oi)
{
	...
	if(indir2_index(n) == 0) 
	{
		uint32_t *indir2 = ospfs_block(oi->oi_indirect2);
		uint32_t offset2 = indir_index(n);
		uint32_t *indir = ospfs_block(indir2[offset2]);
		uint32_t offset = direct_index(n);
		free_block(indir[offset]);
		indir[offset] = 0;
		if(offset == 0)
		{
			indir2[offset2] = 0;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
    ...
    for (i=0; i<dir_oi->oi_size; i+= OSPFS_DIRENTRY_SIZE) {
        ospfs_direntry_t *entry=ospfs_inode_data(dir_oi, i);
        if (entry->od_ino==0){
            return entry;
        }
    }
   
    f_pos=OSPFS_DIRENTRY_SIZE+dir_oi->oi_size;
    retval = change_size(dir_oi, dir_oi->oi_size+OSPFS_BLKBITSIZE);
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
    ...
    if (find_direntry(ospfs_inode(dir->i_ino), dst_dentry->d_name.name, dst_dentry->d_name.len) != 0)
    	return -EEXIST;
...
find_free_inode (void){
    uint32_t free_inode;
    for (free_inode = 2; free_inode < ospfs_super->os_ninodes; free_inode++) {
        ospfs_inode_t *ino = ospfs_inode(free_inode);
        if (ino->oi_nlink==0) 
        	return free_inode;
    }
    return 0;
